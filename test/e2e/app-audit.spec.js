...
test('release metadata and announcement history stay aligned', async ({ request }) => {
    ...
    expect(packageJson.version).toBe('1.0.25');
    expect(packageLock.version).toBe('1.0.25');
    expect(packageLock.packages[''].version).toBe('1.0.25');
    expect(serviceWorker).toContain("const CACHE_NAME = 'mahjong-v26'");
    expect(serviceWorker).toContain("'./assets/ui/icons.svg'");
    expect(serviceWorker).toContain("'./manifest.json'");
    const currentIndex = changelog.indexOf('## [1.0.25] - 2026-09-11');
