...
test('release metadata and announcement history stay aligned', async ({ request }) => {
    ...
    expect(packageJson.version).toBe('1.0.24');
    expect(packageLock.version).toBe('1.0.24');
    expect(packageLock.packages[''].version).toBe('1.0.24');
    expect(serviceWorker).toContain("const CACHE_NAME = 'mahjong-v25'");
    expect(serviceWorker).toContain("'./assets/ui/icons.svg'");
    expect(serviceWorker).toContain("'./manifest.json'");
    const currentIndex = changelog.indexOf('## [1.0.24] - 2026-09-11');
